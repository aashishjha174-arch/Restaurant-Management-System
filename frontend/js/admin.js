...
function renderBookingsList(bookings) {
  ...
  if (bookings.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; padding: 25px;">No reservations found.</td></tr>';
    return;
  }
  bookings.forEach(b => {
    const row = document.createElement('tr');
...
window.deleteBooking = async (id) => {
  ...
  try {
    ...
    if (res.ok) { showToast('Booking deleted', 'success'); loadBookingsTable('all'); }
  } catch (error) { console.error(error); }
};
...
window.toggleMenuItemAvailability = async (id, available) => {
  try {
    ...
    if (res.ok) { showToast('Availability updated', 'success'); loadMenuTable(); }
  } catch (error) { console.error(error); }
};
...
window.deleteGalleryPhoto = async (id) => {
  ...
  try {
    ...
    if (res.ok) { showToast('Photo deleted', 'success'); loadGalleryManager(); }
  } catch (error) { console.error(error); }
};
...
window.deleteReview = async (id) => {
  ...
  try {
    ...
    if (res.ok) { showToast('Review deleted', 'success'); loadReviewsManager(); }
  } catch (error) { console.error(error); }
};
